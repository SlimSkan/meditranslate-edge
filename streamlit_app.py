...
def is_likely_medical(text):
    medical_keywords = [
        ...
        'test', 'scan', 'xray', 'mri', 'ct', 'ultrasound', 'ecg', 'ekg',
        'pain', 'fever', 'cough', 'nausea', 'medication', 'treatment',
    ]
...
with st.expander("📋 See examples"):
    ...
    - Lab results show elevated TSH levels at 8.5 mIU/L
...
if translate_button:
    if not medical_text.strip():
        ...
        if not is_medical:
            st.warning(f"⚠️ May not be medical text ({matches} keywords)")
...
Simple Explanation:"""
            ...
            with output_placeholder.container():
                ...
                if is_medical:
                    st.caption(f"✓ Medical text ({confidence:.0%} confidence)")
